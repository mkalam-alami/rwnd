window.rwnd = (function() {
    ...
    function View(el, stepTemplate) {
        ...
        this.appendStep = function(stateId, step, flags, options) {
            // Default options
            options = options || {};
            ...
            for (var id in db.steps) {
                ...
                for (var targetId in step.links) {
                    if (!db.steps[targetId])
                        throw new Error('Step "' + id + '" links to unknown step "' + targetId + '"');
                ...
                paragraphs.forEach(function(paragraph) {
                   var formattedParagraph = this.markdownParser ? this.markdownParser.makeHtml(paragraph) : paragraph;
                   step.text += '<p>' + formattedParagraph + '</p>'; 
                }, this);
            }
    ...
    function Game(model, view) {
        ...
        this.followLink = function(linkStateId, targetStepId) {
            ...
            // Move to next step
            var state = this.model.pushState(targetStepId, newFlags);
            
            // Update views
            if (!isRewinding && !state.step.disableRewindTo)
                this.view.appendStep(state.id, state.step, state.flags);
            else
                this.view.refresh(this.model.states);
        };
