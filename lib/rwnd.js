window.rwnd = (function() {
    ...
    function View(el, stepTemplate) {
        ...
        this.stepTemplate = window.template(stepTemplate);
        
        this.refresh = function(states) {
            this.el.innerHTML = '';
            states.forEach(function(state) {
                this.appendStep(state.id, state.step, state.flags);
            }, this);
        }
        
        this.appendStep = function(stateId, step, flags) {
            // Filter links to show according to the current flags
            var filteredLinks = {};
            ...
            for (var id in step.links) {
                ...
                if (keep) {
                    ...
                    filteredLinksElIds[id] = this._getElId(stateId, id);
                }
            }
            ...
            var stepHtml = this.stepTemplate(context);
            ...
            for (var id in filteredLinks) {
                var linkEl = document.getElementById(this._getElId(stateId, id));
    ...
    function Model(db) {
        ...
        this.reset = function() {            
            ...
            // DB validation
            if (!this.db.config || !this.db.config.start) console.error('No "config.start" property set');
            if (!this.db.steps) console.error('No "steps" property set');
            if (!this.db.steps[this.db.config.start]) console.error('Starting step "' + this.db.config.start + '" missing');
            for (var id in this.db.steps) {
                var step = this.db.steps[id];
                for (var link in step.links) {
                    if (!this.db.steps[link]) console.error('Step "' + id + '" links to unknown step "' + link + '"');
