window.rwnd = (function() {
    ...
    function View(el, stepTemplate) {
        ...
        this.appendStep = function(stateId, step, flags, animate) {
            ...
            var renderedText = window.Mustache.render(step.text, flagsMap);
    ...
    function Model() {
        this.init = function(db) {   
            // DB validation
            // TODO validate flags
            if (!db.config || !db.config.start) throw new Error('No "config.start" property set');
            if (!db.steps) throw new Error('No "steps" property set');
            if (!db.steps[db.config.start]) throw new Error('Start step "' + db.config.start + '" missing');
            for (var id in db.steps) {
                var step = db.steps[id];
                for (var link in step.links) {
                    if (!db.steps[link]) throw new Error('Step "' + id + '" links to unknown step "' + link + '"');
                ...
                var paragraphs = step.text.trim().replace(/[\n\r]/g, '<br />').split('<br /><br />');
                step.text = '';
                paragraphs.forEach(function(paragraph) {
                   step.text += '<p>' + paragraph + '</p>'; 
                });
            }
    ...
    function Game(model, view) {
        ...
        this.followLink = function(linkStateId, targetStepId) {
            ...
            // Handle rewind
            if (this.model.getLastStat().id != linkStateId)
            ...
            // Update views
            if (isRewinding) {
                this.view.refresh(this.model.states); // TODO support removing steps for performance
            }
            else {
