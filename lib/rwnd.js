...
window.rwnd = (function() {
    ...
    function Model() {
        ...
        this.init = function(db) {
            ...
            if (!db.steps[db.config.initialStep])
                throw new Error('Initial step "' + db.config.start + '" missing');
            ...
            for (var id in db.steps) {
                ...
                step.redirects.forEach(function(redirect, index) {
                    if (!db.redirects[redirect.to])
                        console.error('WARNING: Redirect "' + id + '" links to unknown step "' + targetId + '"');
    ...
    function run(url, options) {
        ...
        download(url, function(status, text) {
            if (xhttp.status == 200)
                runFromData(xhttp.responseText.trim(), options);
